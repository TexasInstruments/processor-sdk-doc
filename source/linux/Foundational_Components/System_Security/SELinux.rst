...
1. On a fresh build, update the config file to use :file:`configs/arago-scarthgap-selinux-config.txt`.

   .. note::

      The default policy is targeted. If you want to update it, add the variable PREFERRED_PROVIDER_virtual/refpolicy = "refpolicy-<your_choice>" where <your_choice> is targeted/mls/mcs in your local_conf file

2. Build your custom image in build directory

   .. code-block:: console

      $ MACHINE=<machine> bitbake -k tisdk-default-image

3. Flash the generated wic image onto your SD card. By default, SELinux functionality is in the disabled state
...
1. Boot the target (in disabled state), Run ``fixfiles -F onboot`` or ``touch /.autorelabel`` to configure GRUB and PAM and to create /.autorelabel.
...
3. Restart the system. If using U-Boot, add ``security=selinux`` to the boot parameters. Temporarily, you can stop at the U-Boot prompt and add,
