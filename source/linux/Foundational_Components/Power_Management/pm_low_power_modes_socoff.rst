...
A typical SOC, from power to HLOS (Linux here for example) takes around 20 seconds to boot (ie reaching till Linux prompt), when it’s ready to
run application.
...
User can select the appropriate low power mode at **build time** to fit the needs of the application.
...
remote firmwares, proceed to enter into standby.
...
IO_ONLY_PLUS_DDR mode will be enabled by changing boardcfg in u-boot at build time.
