...
.. note::
   ...
   The same workflow applies to the entire Sitara MPU family, but for each SoC, specific steps will differ and this document will highlight them
...
- Falcon Mode:
   ...
   This is a feature that allows us to skip **A53 SPL** and **U-boot proper** and jump to TF-A and then the kernel directly saving ~5s in our boot time. Different boot loaders implement it differently.
...
- Using a smaller kernel system
   ...
   -  By default, the kernel image contains many drivers and filesystems to enable the functionality supported for the board but are not necessary for early boot. Trim kernel capabilities by using
