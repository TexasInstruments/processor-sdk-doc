...
.. note::
   ...
   Allocating at u-boot tends to place the CMA in the lower 32 bit region of memory. This is fine if the Wave5 is being
   used in isolated pipelines where the only goal is to encode/decode. If pipelines that incorporate other hardware elements
   are being used, then the correct method would be to allocate at device tree level for compatibility.
